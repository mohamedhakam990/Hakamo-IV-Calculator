...
,
 ...
 private fun toast(s:String)=Toast.makeText(this,s,Toast.LENGTH_SHORT).show()
        ...
        // Hakamo Nursing Hub V7
        val hubTitle = text("مركز التمريض\nHakamo Nursing Hub", 21f, fg(), true)
        hubTitle.setPadding(0, 24, 0, 12)
        brand.addView(hubTitle)

        fun hubCard(title: String, subtitle: String, body: String) {
            val b = Button(this)
            b.text = "$title\n$subtitle"
            b.setAllCaps(false)
        ...
        hubCard("🩺 مركز التقييم", "GCS • NEWS2 • AVPU • الألم • السقوط",
            "أدوات تقييم تمريضي سريعة. استخدم النتيجة مع الحالة السريرية وبروتوكول المنشأة.")

        hubCard("🚨 الطوارئ", "ABCDE • CPR • الصدمة • التشنجات • نقص السكر",
            "مرجع سريع للخطوات الأساسية. في الحالات الطارئة اتبع بروتوكول المستشفى وتعليمات الفريق الطبي.")

        hubCard("💉 الحاسبات المتقدمة", "IV • قطرات • تخفيف • إعادة حل • جرعات حسب الوزن",
            "الحاسبات تعطي نتيجة رياضية فقط. راجع الوصفة والتركيز الفعلي والبروتوكول قبل إعطاء الدواء.")

        hubCard("🧪 دليل التحاليل", "CBC • كلى • كبد • أملاح • تخثر • ABG",
            "مرجع تعليمي للقيم والوحدات والتفسير العام. النطاق المرجعي المطبوع في تقرير المختبر هو المرجع النهائي.")

        hubCard("💊 مركز الأدوية", "الاسم • المادة الفعالة • التصنيف • التحذيرات",
            "معلومات تعليمية عن الأدوية. لا تعتمد على التطبيق وحده لتحديد جرعة أو وصف علاج.")

        hubCard("📝 التوثيق والـHandover", "SBAR • SOAP • Nursing Notes • Handover",
            "قوالب تساعدك في تنظيم وتوثيق المعلومات التمريضية، مع الالتزام بسياسة المنشأة.")

        hubCard("🧠 اختبار التمريض", "Pharmacology • Labs • ICU • أساسيات",
            "قسم تدريبي لأسئلة المراجعة. الهدف التعلم وليس تقييم حالة مريض.")

        // V8 — دليل أقسام التمريض وما يحتاجه كل قسم
        val nursingTitle = text("دليل أقسام التمريض\nNursing Departments Guide", 21f, fg(), true)
        nursingTitle.setPadding(0, 28, 0, 12)
        brand.addView(nursingTitle)

        fun nursingSection(title: String, workText: String, needs: String) {
            val b = Button(this)
            b.text = title
            b.setAllCaps(false)
            b.setOnClickListener {
        ...
        nursingSection("🩺 أساسيات التمريض",
            "تقييم المريض، العلامات الحيوية، إعطاء الأدوية، العناية اليومية، التوثيق والتواصل مع الفريق.",
            "جهاز قياس الضغط، ترمومتر، Pulse oximeter، أدوات قياس السكر، أدوات إعطاء الدواء، PPE، مهارات مكافحة العدوى والتوثيق.")

        nursingSection("🏥 باطني وجراحي",
            "متابعة المرضى قبل وبعد العمليات، الأمراض المزمنة والحادة، الألم، الجروح، السوائل والأدوية.",
            "Vital signs، I&O، wound care، medication safety، متابعة التحاليل، التقييم السريري وخطة الرعاية.")

        nursingSection("🚨 الطوارئ",
            "التعامل الأولي مع الحالات الحرجة، التقييم السريع، الإنعاش، النزيف، الصدمة والحالات الحادة.",
            "ABCDE، CPR/BLS حسب التدريب، Oxygen، suction، IV access، ECG، معدات الطوارئ وPPE.")

        nursingSection("🫀 العناية المركزة ICU",
            "مراقبة المرضى غير المستقرين، السوائل، الأدوية الوريدية، الأجهزة والمراقبة المستمرة.",
            "Cardiac monitor، infusion pumps، ventilator basics، ABG، I&O، pressure monitoring، مهارات مكافحة العدوى.")

        nursingSection("👶 الأطفال Pediatric",
            "رعاية الأطفال، تقييم النمو، السوائل، الأدوية حسب الوزن ومراقبة العلامات الحيوية حسب العمر.",
            "ميزان مناسب للعمر، أدوات قياس الأطفال، pediatric cuffs، حسابات الوزن، growth charts، family education.")

        nursingSection("🍼 حديثي الولادة NICU",
            "رعاية حديثي الولادة ومراقبة الحرارة والتنفس والتغذية والعدوى.",
            "Incubator basics، neonatal monitoring، أدوات أكسجين مناسبة، قياس الحرارة، تغذية آمنة ومكافحة العدوى.")

        nursingSection("🤰 النساء والتوليد",
            "رعاية الحمل والولادة وما بعد الولادة ومراقبة الأم والجنين.",
            "Maternal vital signs، fetal monitoring basics، أدوات الولادة حسب سياسة المكان، postpartum assessment، متابعة النزيف.")

        nursingSection("🎗️ الأورام Oncology",
            "رعاية مرضى الأورام، متابعة الأعراض الجانبية، الألم، التغذية والعدوى، مع الالتزام ببروتوكولات العلاج.",
            "PPE، central-line care، extravasation awareness، infection precautions، symptom assessment، medication safety.")

        nursingSection("❤️ القلب CCU",
            "مراقبة مرضى القلب واضطرابات النظم وألم الصدر وفشل القلب.",
            "ECG basics، cardiac monitor، BP/SpO2، medication safety، I&O، تقييم ألم الصدر ومراقبة التغيرات.")

        nursingSection("🧠 الصحة النفسية",
            "التقييم النفسي، التواصل العلاجي، السلامة ومتابعة الأدوية.",
            "Therapeutic communication، suicide/self-harm risk awareness، medication observation، بيئة آمنة وتوثيق دقيق.")

        nursingSection("🩸 الكلى والغسيل",
            "متابعة مرضى القصور الكلوي والغسيل، السوائل والأملاح والوصول الوعائي.",
            "I&O، daily weight، electrolyte/lab review، مراقبة الوصول الوعائي، infection control، متابعة الضغط.")

        nursingSection("🔪 العمليات OR",
            "التحضير للجراحة، السلامة الجراحية، التعقيم، العدّ والتوثيق والرعاية قبل وبعد الإجراء.",
            "Sterile technique، surgical checklist، PPE، instrument/sponge counts، specimen handling حسب سياسة المنشأة.")

        nursingSection("😴 التخدير",
            "مساندة فريق التخدير ومراقبة المريض قبل وأثناء وبعد التخدير ضمن نطاق الصلاحيات والتدريب.",
            "Airway equipment، oxygen، suction، monitoring، recovery assessment، emergency equipment.")

        nursingSection("🦠 مكافحة العدوى",
            "منع انتقال العدوى، العزل، نظافة اليدين، التعقيم والتعامل الآمن مع الأدوات والنفايات.",
            "Hand hygiene، PPE، isolation precautions، sharps safety، cleaning/disinfection protocols.")

        nursingSection("👴 كبار السن",
            "رعاية كبار السن مع التركيز على الحركة، التغذية، الأدوية، السقوط، الجلد والحالة العقلية.",
            "Fall-risk tools، pressure-injury prevention، medication review، nutrition/hydration، mobility assistance.")

        nursingSection("🌍 صحة المجتمع",
            "التثقيف الصحي، الوقاية، التطعيمات، متابعة الأمراض المزمنة والعمل الميداني.",
            "Health education، screening basics، vaccination records، community assessment، documentation.")

        nursingSection("🫁 العناية التنفسية",
            "متابعة مرضى ضيق التنفس والأكسجين ومراقبة التنفس، مع الالتزام ببروتوكولات الجهاز التنفسي.",
            "SpO2، oxygen devices، suction، respiratory assessment، ABG basics، infection control.")

        // Hakamo watermark/background text inside the app
        val watermark = text("HAKAMO", 54f, fg(), true)
        watermark.alpha = 0.035f
        watermark.gravity = Gravity.CENTER
        watermark.setPadding(0, 24, 0, 24)
        brand.addView(watermark)
